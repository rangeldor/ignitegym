...
export async function storageAuthTokenSave({token, refresh_token}: IStorageAuthTokenProps) {
...
export async function storageAuthTokenGet() {
  ...
  const { token, refresh_token }: IStorageAuthTokenProps = response ? JSON.parse(response) : {}
